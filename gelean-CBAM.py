...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.sigmoid(attention_map) * x
...
# Conv Layer with CBAM integrated
class ConvWithCBAM(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.cbam = CBAM(c2)  # Add CBAM after the convolution

    def forward(self, x):
        x = self.conv(x)
        x = self.bn(x)
        x = self.act(x)
        x = self.cbam(x)  # Apply CBAM after the activation
        return x
